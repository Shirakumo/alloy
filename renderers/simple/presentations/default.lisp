...
(defmethod alloy:compute-ideal-size ((layout-element alloy:renderable) (sizing-strategy fit-to-shapes) (size alloy:size))
  (let ((shapes (shapes sizing-strategy))
    ...
    (flet ((process-shape (shape)
             (when shape
               (let* ((shape-offset (offset shape))
                      (shape-min-x (alloy:pxx shape-offset))
                      (shape-min-y (alloy:pxy shape-offset))
                 ...
                 (setf min-x (if (null min-x) shape-min-x (min min-x shape-min-x)))
                 (setf min-y (if (null min-y) shape-min-y (min min-y shape-min-y)))
