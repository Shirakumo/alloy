  ...
  (alloy:with-unit-parent element
    (let ((bounds (alloy:bounds element)))
      ...
      (alloy:extent (alloy:margins-l margins)
                    ...
                    (alloy:u- (alloy:extent-w bounds)
                              (alloy:margins-l margins)
                              (alloy:margins-r margins))
                    (alloy:u- (alloy:extent-h bounds)
                              (alloy:margins-b margins)
                              (alloy:margins-u margins))))))
...
(defmethod render-inside ((renderer renderer) (text text) renderable)
  (let ((extent (to-extent text renderable)))
    ...
    (let ((point (alloy:point (alloy:u+ (alloy:extent-x extent)
                                        (ecase (halign text)
                                          (:start 0)
                                          (:middle (/ (alloy:to-px (alloy:extent-w extent)) 2))
                                          (:end (alloy:extent-w extent))))
                              (alloy:u+ (alloy:extent-y extent)
                                        (ecase (valign text)
                                          (:bottom 0)
                                          (:middle (/ (alloy:to-px (alloy:extent-h extent)) 2))
...
(defmethod render-inside ((renderer renderer) (icon icon) renderable)
  (let ((extent (to-extent icon renderable))
    ...
    (let ((point (alloy:point (alloy:u+ (alloy:extent-x extent)
                                        (ecase (halign icon)
                                          (:start 0)
                                          (:middle (/ (alloy:to-px (alloy:u- (alloy:extent-w extent) (alloy:size-w size))) 2))
                                          (:end (alloy:u- (alloy:extent-w extent) (alloy:size-w size)))))
                              (alloy:u+ (alloy:extent-y extent)
                                        (ecase (valign icon)
                                          (:bottom 0)
                                          (:middle (/ (alloy:to-px (alloy:u- (alloy:extent-h extent) (alloy:size-h size))) 2))
                                          (:top (alloy:u- (alloy:extent-h extent) (alloy:size-h size))))))))
