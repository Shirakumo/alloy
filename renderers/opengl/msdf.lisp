                        ...
                        (case c
                          ...
                          (t
                           (let ((char (gethash c chars +default-char+))
                                 (k (* scale (kerning p c))))
                             (incf x k)
                             (let ((x- (+ x (* scale (getf char :xoffset))))
                                   (y- (* scale (getf char :yoffset)))
                                   (x+ (+ x (* scale (+ (getf char :xoffset) (getf char :width)))))
                                   (y+ (* scale (+ (getf char :yoffset) (getf char :height))))
                                   (u- (/ (getf char :x) w))
                                   (v- (/ (getf char :y) h))
...
(defmethod alloy:suggest-bounds (extent (text text))
  (let ((scale (scale text))
    ...
    (multiple-value-bind (breaks array font-sequence x- y- x+ y+) (compute-text (simple:font text) (alloy:text text) extent (alloy:to-px (simple:size text)) (simple:wrap text) markup)
      (let* ((w (- x+ x-))
             (h (- y+ y-))
