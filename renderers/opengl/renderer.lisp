...
(defmethod render-direct ((shape line-strip) renderer color)
  (let ((shader (resource 'line-shader renderer))
    ...
    (flet ((prepare (shader)
      ...
      (draw-vertex-array (resource 'line-vao renderer) :triangles 0 (min (floor (length lines) 5)
                                                                         (* (or (size shape) 1000000) 6)))
  ...
  (let* ((corner-radii (simple:corner-radii shape))
    ...
    (flet ((prepare (shader)
             ...
             (let ((min (min (alloy:pxw extent) (alloy:pxh extent))))
               ...
               (setf (uniform shader "corner_radius[3]") (if round-p (min min (alloy:to-px (aref corner-radii 2))) 0.0))
               (setf (uniform shader "corner_radius[4]") (if round-p (min min (alloy:to-px (aref corner-radii 3))) 0.0)))
