...
(defmethod initialize-instance :after ((keyboard virtual-keyboard) &key (keyboard-spec :100%))
  (setf (layout keyboard) keyboard-spec))

(defmethod reinitialize-instance :after ((keyboard virtual-keyboard) &key keyboard-spec)
  (when keyboard-spec (setf (layout keyboard) keyboard-spec)))
...
(defmethod handle ((event button-event) (layout virtual-keyboard))
  (flet ((send (target)
    ...
    (case (button event)
      ...
      (:b (exit layout))
      (:x (send (index-element :backspace layout)))
      (:y (send (index-element :space layout)))
      (:l3 (send (index-element :caps-lock layout)))
      (:l2 (send (index-element :left-shift layout)))
