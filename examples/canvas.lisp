...
(define-example canvas (screen)
  (let* ((window (windowing:make-window screen :min-size (alloy:px-size 640 480)))
    ...
    (simple:fill-rectangle canvas 10 15 100 30 '(0.0 10.0 5.0 2.0))
    ...
    (flet ((circles (a b)
             ...
             (simple:fill-ellipse canvas 20 30 20 20)
             (setf (simple:pattern canvas) colors:green)
             (simple:fill-ellipse canvas 22 32 16 16 a b)
             (simple:draw-ellipse canvas 20 30 20 20 a b)
    ...
    (flet ((circles ()
             (setf (simple:pattern canvas) (colored:color 1.0 1.0 0.0 0.5))
             (simple:fill-ellipse canvas 80 30 20 20 0.0 (* 0.5 PI))
             (setf (simple:pattern canvas) (colored:color 1.0 0.0 1.0 0.5))
             (simple:fill-ellipse canvas 90 30 20 20 0.0 (* 0.5 PI))
             (setf (simple:pattern canvas) (colored:color 0.0 1.0 1.0 0.5))
             (simple:fill-ellipse canvas 85 40 20 20 0.0 (* 0.5 PI))))
      (setf (simple:pattern canvas) colors:black)
      ...
      (circles)
      (setf (simple:composite-mode canvas) :add)
      (simple:translate canvas (alloy:point 0 40))
      (circles)
      ...
      (simple:pop-matrix canvas))
    (flet ((line ()
             (simple:translate canvas (alloy:point 0 10))
