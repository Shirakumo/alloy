...
(define-example canvas (screen)
  (let* ((window (windowing:make-window screen :min-size (alloy:px-size 640 480)))
    ...
    (setf (simple:line-width canvas) 50.0)
    (simple:translate canvas (alloy:point 200 0))
    (simple:start-line canvas 0 0)
    (simple:move-to canvas 100 0)
    (simple:move-to canvas 100 100)
    (simple:move-to canvas 150 0)
    (simple:complete-shape canvas)))
