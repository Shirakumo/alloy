...
(defmethod (setf cell-margins) :after (value (layout border-layout))
  (setf (layout-needed-p layout) T))

(defmethod (setf padding) :after (value (layout border-layout))
  (setf (layout-needed-p layout) T))
...
(defmethod refit ((layout border-layout))
  (macrolet ((with-border ((slot dim) &body body)
               `(destructuring-bind (&optional element size) (slot-value layout ',slot)
                  (when element
                    (let ((size (to-px (or size (,dim element)))))
                      ,@body)))))
    (with-unit-parent layout
      (destructure-margins (:l ml :u mu :r mr :b mb :to-px T) (cell-margins layout)
        (let* ((extent (bounds layout))
          ...
          ;;(decf h (+ (pxb p) (pxu p)))
          (with-border (b pxh)
            (let* ((size (- (min h size) mu mb))
                   (diff (max size (pxh (suggest-size (px-size (- w ml mr) size) element)))))
              (setf (bounds element) (px-extent (+ x ml) (+ y mb) (- w ml mr) diff))
              (decf h diff)
              (incf y diff)))
          (with-border (u pxh)
            (let* ((size (- (min h size) mu mb))
                   (diff (max size (pxh (suggest-size (px-size (- w ml mr) size) element)))))
              (setf (bounds element) (px-extent (+ x ml) (+ y (- h diff mu) mb) (- w ml mr) diff))
              (decf h diff)))
          (setf h (umax 0 h))
          (with-border (l pxw)
            (let* ((size (- (min w size) ml mr))
                   (diff (max size (pxw (suggest-size (px-size size (- h mu mb)) element)))))
              (setf (bounds element) (px-extent (+ x ml) (+ y mb) diff (- h mu mb)))
              (decf w diff)
              (incf x diff)))
          (with-border (r pxw)
            (let* ((size (- (min w size) ml mr))
                   (diff (max size (pxw (suggest-size (px-size size (- h mu mb)) element)))))
              (setf (bounds element) (px-extent (+ x (- w diff ml mr) ml) (+ y mb) diff (- h mu mb)))
              (decf w diff)))
          (setf w (umax 0 w))
          (with-border (c pxh)
            (declare (ignore size))
            (setf (bounds element) (px-extent (+ x (if (slot-value layout 'l) (pxl p) 0) ml)
                                              (+ y (if (slot-value layout 'b) (pxb p) 0) mb)
                                              (- w (if (slot-value layout 'l) (pxl p) 0)
                                                 (if (slot-value layout 'r) (pxr p) 0) ml mr)
                                              (- h (if (slot-value layout 'b) (pxb p) 0)
                                                 (if (slot-value layout 'u) (pxu p) 0) mu mb)))))))))
  ...
  (macrolet ((with-border ((slot dim) &body body)
    ...
    (with-unit-parent layout
      (let ((w 0) (h 0) (x 0) (y 0))
        (destructure-margins (:l ml :u mu :r mr :b mb :to-px T) (cell-margins layout)
          (with-border (b pxh)
            (incf h (+ mu mb (pxh (suggest-size (px-size (w new-size) size) element))))
            (incf y h))
          (with-border (l w)
            (let ((size (suggest-size (px-size size (- (pxh new-size) h)) element)))
              (incf w (+ ml mr (pxw size)))
              (incf x (+ ml mr (pxw size)))
              (setf h (max h (+ mu mb y (pxh size))))))
          (with-border (c w)
            (let ((size (suggest-size (px-size (if (= 0 (to-px size)) (- (pxw new-size) w) size) (- (pxh new-size) h)) element)))
              (incf w (+ ml mr (pxw size)))
              (incf x (+ ml mr (pxw size)))
              (setf h (max h (+ mu mb y (pxh size))))))
          (with-border (r w)
